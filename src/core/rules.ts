import { CompressionRule, ParsedDoc, SectionType } from './types';
...
export const abbreviateCommonTerms: CompressionRule = {
    ...
    const processedSections = doc.sections.map(section => ({
      ...
      children: section.children 
        ? section.children.map(child => ({
            ...child,
            content: processContent(child.content)
          }))
        : undefined
    }));
...
export const removeRedundantDocs: CompressionRule = {
  ...
  apply(doc: ParsedDoc): ParsedDoc {
    ...
    const redundantSectionTypes: SectionType[] = [
    ...
    const filteredSections = doc.sections.filter(section => {
      ...
      const isRedundant = 
        section.content.toLowerCase().includes(`the ${name}`) ||
...
export const compressFunctionSignatures: CompressionRule = {
  ...
  weight: 60,
  apply(doc: ParsedDoc): ParsedDoc {
    const processFunctionSignature = (content: string): string => {
    ...
    const processedSections = doc.sections.map(section => {
      if (section.type === 'function-signature' || section.type === 'code') {
        return {
          ...section,
          content: processFunctionSignature(section.content)
        };
      }
      return section;
    });
...
export const compressCodeExamples: CompressionRule = {
  ...
  weight: 30,
  apply(doc: ParsedDoc): ParsedDoc {
    const processCodeExample = (content: string): string => {
      return content
    ...
    const processedSections = doc.sections.map(section => {
      if (section.type === 'code' || section.type === 'example') {
        return {
          ...section,
          content: processCodeExample(section.content)
        };
      }
      return section;
    });
...
export const compressHeadings: CompressionRule = {
  ...
  weight: 70,
  apply(doc: ParsedDoc): ParsedDoc {
    const processHeading = (content: string): string => {
    ...
    const processedSections = doc.sections.map(section => {
      if (section.type === 'heading') {
        return {
          ...section,
          content: processHeading(section.content)
        };
      }
      return section;
    });
...
export const compressApiEndpoints: CompressionRule = {
  ...
  weight: 65,
  apply(doc: ParsedDoc): ParsedDoc {
    const processApiEndpoint = (content: string): string => {
    ...
    const processedSections = doc.sections.map(section => {
      if (section.type === 'api-endpoint') {
        return {
          ...section,
          content: processApiEndpoint(section.content)
        };
      }
      return section;
    });
...
export const defaultRules: CompressionRule[] = [
  abbreviateCommonTerms,
  removeRedundantDocs,
  compressFunctionSignatures,
  compressCodeExamples,
  compressHeadings,
  compressApiEndpoints
];

// Export a map of rules by ID for easy lookup
export const rulesById = defaultRules.reduce<Record<string, CompressionRule>>(
  (acc, rule) => {
    acc[rule.id] = rule;
    return acc;
  },
